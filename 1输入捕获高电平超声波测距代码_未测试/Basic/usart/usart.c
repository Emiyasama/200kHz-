...
extern u8 c;
extern u8 fashe[256];//发射
extern u16 n; //这里伍明慧做了修改：因为u8能存储的最大值就是256，所以u8的采集点n永远小于256
...
void USART3_Init(u32 BaudRate){ //USART3初始化并启动
   ...
   //串口使用的GPIO口配置
   GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;//设置USART3的RX接口是PB11—-TXD
   GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//接口模式 浮空输入
   GPIO_Init(GPIOB, &GPIO_InitStructure);

   GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;//设置USART3的TX接口是PB10--RXD
